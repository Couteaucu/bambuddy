...
class TestEnrichRemainFromInventory:
    """Tests for _enrich_remain_from_inventory."""
    ...
    @pytest.mark.asyncio
    async def test_enriches_assigned_slot(self, scheduler):
        """Inventory fill overwrites MQTT remain for a slot with an assignment."""
        from unittest.mock import AsyncMock, MagicMock

        # Spool: 1000g label, 250g core, 400g used → (1000 - 400 - 250) / 1000 * 100 = 35%
        mock_spool = MagicMock()
        mock_spool.label_weight = 1000
        mock_spool.core_weight = 250
        mock_spool.weight_used = 400.0
        mock_spool.last_scale_weight = None

        mock_assignment = MagicMock()
        mock_assignment.ams_id = 0
        mock_assignment.tray_id = 1

        mock_result = MagicMock()
        mock_result.all.return_value = [(mock_assignment, mock_spool)]

        db = AsyncMock()
        db.execute = AsyncMock(return_value=mock_result)

        loaded_filaments = [
            {"ams_id": 0, "tray_id": 1, "global_tray_id": 1, "remain": 80},
            {"ams_id": 0, "tray_id": 2, "global_tray_id": 2, "remain": 60},
        ]

        await scheduler._enrich_remain_from_inventory(db, printer_id=1, loaded_filaments=loaded_filaments)

        assert loaded_filaments[0]["remain"] == 35  # enriched
        assert loaded_filaments[1]["remain"] == 60  # no assignment — unchanged

    @pytest.mark.asyncio
    async def test_uses_last_scale_weight_when_available(self, scheduler):
        """If last_scale_weight is set, use scale-based fill instead of weight_used."""
        from unittest.mock import AsyncMock, MagicMock

        # Spool: 1000g label, 250g core, last_scale=600g → (600 - 250) / 1000 * 100 = 35%
        mock_spool = MagicMock()
        mock_spool.label_weight = 1000
        mock_spool.core_weight = 250
        mock_spool.weight_used = 0.0
        mock_spool.last_scale_weight = 600

        mock_assignment = MagicMock()
        mock_assignment.ams_id = 0
        mock_assignment.tray_id = 0
        ...
        loaded_filaments = [
            {"ams_id": 0, "tray_id": 0, "global_tray_id": 0, "remain": 99},
        ]

        await scheduler._enrich_remain_from_inventory(db, printer_id=1, loaded_filaments=loaded_filaments)

        assert loaded_filaments[0]["remain"] == 35  # (600 - 250) / 1000 * 100

    @pytest.mark.asyncio
    async def test_no_assignments_leaves_remain_unchanged(self, scheduler):
        """When no inventory assignments exist, MQTT remain values are preserved."""
        from unittest.mock import AsyncMock, MagicMock
        ...
        loaded_filaments = [
            {"ams_id": 0, "tray_id": 0, "global_tray_id": 0, "remain": 80},
            {"ams_id": 0, "tray_id": 1, "global_tray_id": 1, "remain": 30},
        ]

        await scheduler._enrich_remain_from_inventory(db, printer_id=1, loaded_filaments=loaded_filaments)

        assert loaded_filaments[0]["remain"] == 80
        assert loaded_filaments[1]["remain"] == 30

    @pytest.mark.asyncio
    async def test_fill_clamped_to_zero(self, scheduler):
        """Inventory fill is never negative (e.g. over-counted weight_used)."""
        from unittest.mock import AsyncMock, MagicMock

        # weight_used exceeds label — fill should clamp to 0
        mock_spool = MagicMock()
        mock_spool.label_weight = 1000
        mock_spool.core_weight = 250
        mock_spool.weight_used = 1100.0
        mock_spool.last_scale_weight = None
        ...
        loaded_filaments = [
            {"ams_id": 0, "tray_id": 0, "global_tray_id": 0, "remain": 5},
        ]

        await scheduler._enrich_remain_from_inventory(db, printer_id=1, loaded_filaments=loaded_filaments)

        assert loaded_filaments[0]["remain"] == 0  # clamped
